...
1. Pin never modifies any value in ns; read-only
2. Variable names not in ns are rendered as `[name: not found]`; no exception raised
3. _signal() output is alphabetically sorted by variable name, ensuring stable output
4. When ns is None, _signal() returns empty string without error
5. Single file must not exceed 100 lines (skill.py)
