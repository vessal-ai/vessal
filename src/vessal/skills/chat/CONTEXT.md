...
1. inbox is a transient queue, not restored from JSONL on restart (unread_count resets to zero after restart)
2. chat.jsonl is append-only, never overwritten
3. After read() or reply(), _inbox is cleared and unread_count resets to zero — reply is acknowledgment
4. receive() and reply() must write to chat.jsonl synchronously; no async delayed writes
5. Single file must not exceed 500 lines
6. reply(wait=N) returns only messages that arrived after read(), not old messages
