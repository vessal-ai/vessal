...
```
__init__.py          __init__.py — Hull public interface: Agent orchestration layer entry point.
cell/  Single-frame SORA execution engine. Encapsulates Kernel (execution) and Core (reasoning); drives one Ping, Core, Pong, Kernel, FrameRecord cycle via step().
event_loop.py        event_loop.py — Hull event loop: drives the Agent sleep-wake lifecycle and frame execution.
hull.py              hull.py — Hull configuration layer and lifecycle manager: reads hull.toml, initializes Cell, drives event loop.
hull_api.py          hull_api.py — Interface for Skill servers to interact with Hull: route registration, wake, and static responses.
prompts/
skill.py             skill.py — Skill abstract base class: defines the interface contract that all Skills must implement.
skill_manager.py     skill_manager.py — Skill lifecycle management: discovers, loads, and unloads Skill packages.
skills_manager.py    skills_manager.py — SkillsManager: the Skill management interface that Agents call in the namespace.
tests/
```
