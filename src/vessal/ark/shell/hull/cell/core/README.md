...
```
__init__.py          __init__.py — Core public interface: LLM call pipeline.
core.py              core.py — LLM call pipeline: Core is the inference half of the Agent loop, responsible for model calls and retries.
parser.py            parser.py — LLM response parser (pure function).
retry.py             retry.py — LLM API call retry helper functions (pure functions, no side effects).
tests/
```
