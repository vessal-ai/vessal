...
1. Single file must not exceed 300 lines
2. Gate only checks, does not modify incoming content (reject-only, no content transformation)
3. auto mode must have zero overhead: no rules are executed, returns immediately
4. Rule function signature is fixed: `(content: str) -> str | None`, None means pass, string means rejection reason
5. Rule execution exceptions must be caught + logged, not propagated outward (a single rule crash does not affect other rules)
6. All public interfaces must have complete docstrings and type annotations
