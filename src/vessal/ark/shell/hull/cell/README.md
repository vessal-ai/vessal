...
Responsible for:
- Single-frame execution orchestration (step()): kernel.ping(None, ns) bootstrap → state_gate → core.step → action_gate → kernel.ping(pong, ns)
...
- Bootstrap: first call only invokes kernel.ping(None, ns) to produce the initial Ping before any LLM call
...
sequenceDiagram
    ...
    Cell->>Kernel: kernel.ping(None, ns) → Ping
    ...
    Cell->>Kernel: kernel.ping(pong, ns) → Ping (next frame)
