...
```
__init__.py          __init__.py — Shell public interface: HTTP boundary layer entry point.
cli.py               cli.py — Vessal CLI entry point: start / stop / send / status / read / init / skill subcommands.
container/  Docker container entry adapter. When Vessal runs as a Docker container, this module provides the bootstrap program, creates a Hull instance, and starts the HTTP service.
hull/  Agent runtime orchestrator. Reads hull.toml config, initializes Cell, manages Skill lifecycle, event loop, and meta-skill scheduling.
hull_runner.py       hull_runner.py — Hull subprocess entry point: creates Hull + internal HTTP server + event loop.
protocol.py          protocol.py — Shell ↔ Hull protocol definitions. Shared by all Shell implementations.
server.py            server.py — Shell HTTP gateway: manages Hull subprocess + HTTP reverse proxy.
tests/
```
