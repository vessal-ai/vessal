# Container

Docker container entry adapter. When Vessal runs as a Docker container, this module provides the bootstrap program, creates a Hull instance, and starts the HTTP service.

Responsible for:
- Hull instance creation and initialization (reads hull.toml from project_dir)
- HTTP server startup (binds to 0.0.0.0, externally accessible across the container)
- HTTP request routing and forwarding to Hull (for all non-/healthz routes)
- /healthz health check endpoint (does not go through Hull)
- SIGTERM signal handling (graceful shutdown)
- Image → volume file sync (sync_image_to_volume): full copy on first startup, image updates overwrite definition files, runtime data is untouched

Not responsible for:
- Agent execution (handled by Hull)
- Skill management (handled by Hull)
- Logging system (handled by vessal.util.logging; this module only outputs to stdout)

## Constraints

1. HTTP service binds to 0.0.0.0 (externally accessible from outside the container), not 127.0.0.1
2. No subprocess calls — Shell logic is in Hull, not here
3. All errors return valid HTTP responses; no exceptions propagate out of the container
4. Graceful shutdown: SIGTERM triggers Hull.stop(), HTTP service waits for existing requests to complete before closing
5. `_ContainerHandler` and `hull_runner._HullHandler` must maintain consistent parameter signatures
6. `sync_image_to_volume` does not delete files in the volume that don't exist in the image (Agent-created skills, runtime data)

## Design

Container is a direct container wrapper around Hull — it only does initialization and HTTP proxying; all business logic is Hull's responsibility. Differences from hull_runner.py:

1. **Network binding**: hull_runner binds to 127.0.0.1 (Shell internal communication), Container binds to 0.0.0.0 (external container access)
2. **Signal handling**: hull_runner prints READY after handling SIGTERM, Container calls Hull.stop() directly
3. **Log output**: hull_runner outputs the READY signal to stdout for Shell to parse, Container outputs logs to docker logs

`_ContainerHandler.do_GET()` and `do_POST()` call `Hull.handle()`, which returns a `(status, data)` pair, then call `_respond()` or `_respond_json()` to serialize. The signatures of the two response methods follow Formalin DTC: data first, status optional (default 200).

```mermaid
graph LR
    Client["Docker external client\n(docker run -p)"]
    Handler["_ContainerHandler"]
    Hull["Hull\n(processing logic)"]

    Client -->|"HTTP GET/POST"| Handler
    Handler -->|"forward request"| Hull
    Hull -->|"return (status, data)"| Handler
    Handler -->|"serialize response"| Client
```

## Public Surface

- `main()` — container entry point; parses arguments (--dir, --port), creates Hull, starts HTTP, runs event loop
- `_ContainerHandler` — HTTP request handler; forwards to Hull or responds to /healthz
- `sync_image_to_volume(image_src, volume_dst)` — sync image files to volume (full copy on first startup / selective overwrite on image update)

## Status

### TODO
None.

### Known Issues
None.

### Active
None.

### Completed
- 2026-04-13: Parameter signature alignment — `_respond()` and `_respond_json()` changed to `(data, status=200)` format, consistent with `hull_runner._HullHandler`
- 2026-04-14: Volume persistence fix — mount point expanded from /app/agent/data to /app/agent; added `sync_image_to_volume` to handle image → volume file sync
