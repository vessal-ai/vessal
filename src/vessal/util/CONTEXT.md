...
1. Does not directly import Shell, Hull, or Cell layer code (dependency direction: upper layers call Util, not the reverse)
2. All functions must be pure functions (no side effects, no global state)
3. No external dependencies (standard library only)
4. No single file exceeds 300 lines
5. All public functions must have complete docstrings and type annotations
