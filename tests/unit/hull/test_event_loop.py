"""test_event_loop — Hull event loop unit tests."""
import asyncio
import pytest
from unittest.mock import MagicMock, patch

from vessal.hull.event_loop import EventLoop


@pytest.fixture
def mock_cell():
    from vessal.skills.system.skill import System

    class _FakeKernel:
        def __init__(self):
            self.L = {
                "_frame": 0,
                "signals": {},
            }

    fk = _FakeKernel()
    sys_skill = System()
    sys_skill._bind_kernel(fk)
    cell = MagicMock()
    cell.L = fk.L
    cell.G = {"_system": sys_skill}
    return cell


@pytest.fixture
def mock_compaction_cell():
    return MagicMock()


def _make_loop(mock_cell, mock_compaction_cell, **kwargs):
    """Construct EventLoop with the dual-cell signature."""
    return EventLoop(
        main_cell=mock_cell,
        compaction_cell=mock_compaction_cell,
        main_db_path="",
        **kwargs,
    )


def test_inject_wake_sets_namespace(mock_cell, mock_compaction_cell):
    """inject_wake writes the wake reason onto _system Skill."""
    loop = _make_loop(mock_cell, mock_compaction_cell)
    loop.inject_wake({"reason": "user_message"})
    assert mock_cell.G["_system"]._wake_reason == "user_message"


def test_inject_wake_clears_idle(mock_cell, mock_compaction_cell):
    """inject_wake clears the _sleeping flag on the _system Skill."""
    mock_cell.G["_system"]._sleeping = True
    loop = _make_loop(mock_cell, mock_compaction_cell)
    loop.inject_wake({"reason": "heartbeat"})
    assert mock_cell.G["_system"]._sleeping is False


def test_inject_wake_default_reason(mock_cell, mock_compaction_cell):
    """Uses heartbeat when the event has no reason field."""
    loop = _make_loop(mock_cell, mock_compaction_cell)
    loop.inject_wake({})
    assert mock_cell.G["_system"]._wake_reason == "heartbeat"


def test_inject_wake_does_not_touch_skills(mock_cell, mock_compaction_cell):
    """inject_wake no longer looks up or calls any Skill."""
    from vessal.skills.chat.skill import Chat
    h = Chat()
    mock_cell.L["chat"] = h
    loop = _make_loop(mock_cell, mock_compaction_cell)
    loop.inject_wake({"reason": "user_message", "content": "hello"})
    # chat skill inbox should have no messages (inject_wake does not deliver)
    assert h.read() == []


def test_run_wake_cycle_skips_frame_logger_without_log_dir(mock_cell, mock_compaction_cell):
    """_run_wake_cycle does not create a FrameLogger when Tracer has no log directory."""
    from vessal.hull.event_loop import FrameHooks
    from vessal.util.logging import Tracer

    mock_cell.G["_system"]._sleeping = True  # sleep immediately, zero frame loop

    loop = _make_loop(
        mock_cell,
        mock_compaction_cell,
        tracer=Tracer("", enabled=False),
    )
    # Verify no error and no file created
    loop._run_wake_cycle()  # should complete without touching filesystem


def test_inject_wake_alarm_reason(mock_cell, mock_compaction_cell):
    """alarm wake writes _system._wake_reason = 'alarm'."""
    loop = _make_loop(mock_cell, mock_compaction_cell)
    loop.inject_wake({"reason": "alarm"})
    assert mock_cell.G["_system"]._wake_reason == "alarm"


def test_frame_loop_calls_before_frame_hook(mock_cell, mock_compaction_cell, monkeypatch):
    """_frame_loop calls hooks.before_frame every frame."""
    from vessal.hull.event_loop import FrameHooks

    monkeypatch.setattr(
        "vessal.hull.event_loop.should_compact",
        lambda path, k=4: False,
    )

    call_count = {"n": 0}

    def before():
        call_count["n"] += 1

    hooks = FrameHooks(before_frame=before)

    # Agent sleeps immediately after one frame
    def step_then_idle(tracer=None):
        mock_cell.G["_system"]._sleeping = True
        r = MagicMock()
        r.protocol_error = None
        return r

    mock_cell.G["_system"]._sleeping = False
    mock_cell.step = step_then_idle

    loop = _make_loop(mock_cell, mock_compaction_cell, hooks=hooks)
    loop._frame_loop()

    assert call_count["n"] == 1


def test_event_queue_is_stdlib_queue(mock_cell, mock_compaction_cell):
    """event_queue is a stdlib queue.Queue (thread-safe)."""
    import queue as queue_mod
    loop = _make_loop(mock_cell, mock_compaction_cell)
    assert isinstance(loop.event_queue, queue_mod.Queue)


def test_inject_wake_fn_puts_event_on_queue():
    """ns['_inject_wake'] injects a wake event onto the event queue."""
    import queue as queue_mod
    from unittest.mock import MagicMock
    cell = MagicMock()
    cell.L = {}
    compaction_cell = MagicMock()
    loop = EventLoop(main_cell=cell, compaction_cell=compaction_cell, main_db_path="")
    # Simulate Hull exposing _inject_wake
    inject_fn = lambda reason="user_message": loop.event_queue.put({"reason": reason})
    cell.L["_inject_wake"] = inject_fn

    inject_fn("user_message")
    event = loop.event_queue.get(timeout=1)
    assert event["reason"] == "user_message"


def test_after_frame_hook_fires(mock_cell, mock_compaction_cell, monkeypatch):
    """_frame_loop calls hooks.after_frame once per successful frame."""
    from vessal.hull.event_loop import FrameHooks

    monkeypatch.setattr(
        "vessal.hull.event_loop.should_compact",
        lambda path, k=4: False,
    )

    calls = {"n": 0}

    def after():
        calls["n"] += 1

    hooks = FrameHooks(after_frame=after)

    def step_then_idle(tracer=None):
        mock_cell.G["_system"]._sleeping = True
        r = MagicMock()
        r.protocol_error = None
        return r

    mock_cell.G["_system"]._sleeping = False
    mock_cell.step = step_then_idle

    loop = _make_loop(mock_cell, mock_compaction_cell, hooks=hooks)
    loop._frame_loop()

    assert calls["n"] == 1


def test_frame_loop_breaks_on_protocol_error(mock_cell, mock_compaction_cell):
    """_frame_loop breaks immediately on protocol_error without retrying."""
    from vessal.cell.protocol import StepResult

    call_count = 0

    def fake_step(tracer=None):
        nonlocal call_count
        call_count += 1
        return StepResult(protocol_error="BadRequestError: test")

    mock_cell.G["_system"]._sleeping = False
    mock_cell.step = fake_step
    loop = _make_loop(mock_cell, mock_compaction_cell, max_frames_per_wake=10)
    loop._frame_loop()

    assert call_count == 1, f"expected 1 call, got {call_count}"
    assert mock_cell.G["_system"]._sleeping is True
