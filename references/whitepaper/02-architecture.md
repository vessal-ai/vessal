...
`step(tracer) → StepResult` is the core method. It executes a single frame: render Ping → call LLM for Pong → gate validation → execute code → commit frame record. Returns StepResult (success or protocol error). `run_forever()` is the long-running loop driven by Hull's EventLoop; `step()` is the unit Hull calls per frame. These names are enforced by `tests/architecture/vessal/test_cell_dependency_tree.py`.
...
```mermaid
flowchart LR
    Hull["Hull\n(imports concrete Tracer)"] -->|"cell.step(tracer=tracer)"| Cell["Cell"]
    Cell -->|"TracerLike Protocol\n(_tracer_protocol.py)"| TP["structural match\nno import of ark.util.logging"]
    style TP fill:#fffde7,stroke:#f9a825
```

**Snapshot and Skill metadata.** `Cell.snapshot` and `Cell.restore` handle only cloudpickle-serialized namespace bytes. Skill lifecycle metadata is Hull's responsibility. Before snapshotting, Hull writes a sibling `<snap>.skills.json` manifest; before restoring, Hull reads it and primes `sys.path` and `sys.modules` so cloudpickle can resolve Skill classes. Cell is unaware of this file. This split preserves Cell's leaf status: Cell knows how to serialize Python objects; Hull knows which Skill packages must be loadable at restore time.
...
sequenceDiagram
    ...
    Hull->>Hull: write <snap>.skills.json (Skill paths + names)
    Hull->>Cell: cell.snapshot(path)
    Cell->>Cell: cloudpickle namespace → <snap>.pkl

    Note over Hull,Cell: --- crash / restart ---

    Hull->>Hull: read <snap>.skills.json → prime sys.path/sys.modules
    Hull->>Cell: cell.restore(path)
    Cell->>Cell: cloudpickle loads <snap>.pkl (Skill classes now resolvable)
```
...
- `snapshot(path) / restore(path)` — serialize / deserialize namespace bytes only (see §2.2 for Skill manifest ownership)
- `ping / pong` — read-only projections of the most recently committed FrameRecord; updated at the end of each `step()`
